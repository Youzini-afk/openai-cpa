...
async def lifespan(app: FastAPI):
    try:
        reload_all_configs()
        if (
            str(getattr(core_engine.cfg, "PROXY_BACKEND_MODE", "") or "").strip().lower() == "embedded_mihomo"
            and bool(getattr(core_engine.cfg, "EMBEDDED_MIHOMO_ENABLE", False))
        ):
            get_embedded_mihomo_manager().start()
    except Exception as exc:
        print(f"[{core_engine.ts()}] [WARNING] 内置 Mihomo 启动检查失败: {exc}", flush=True)
    yield
    print("\n" + "="*65, flush=True)
    ...
    try:
        if bool(getattr(core_engine.cfg, "EMBEDDED_MIHOMO_ENABLE", False)):
                        ...
                        while True:
                            ...
                            stats_payload = {
                                ...
                                "is_running": is_running,
                                 "mode": "CPA仓管" if getattr(core_engine.cfg, 'ENABLE_CPA_MODE', False) else ("Sub2Api" if getattr(core_engine.cfg, 'ENABLE_SUB2API_MODE', False) else ("Neuralwatt" if getattr(core_engine.cfg, 'ENABLE_NEURALWATT_MODE', False) else "常规量产"))
                             }
                            try:
